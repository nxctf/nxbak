...
def test_restore_metadata_execution(monkeypatch, tmp_path: Path):
    backup = tmp_path / "database.dump.gz"
    backup.write_bytes(b"dump")
    manifest = create_manifest(backup_type="manual", encrypted=False, backup_files=[backup], created_at="2026-06-07T02:00:00Z")
    manifest["metadata"] = {
        "extensions": [{"name": "uuid-ossp", "schema": "extensions"}],
        "realtime_tables": ["public.solves"]
    }
    write_manifest(tmp_path / "manifest.json", manifest)
    (tmp_path / "checksums.sha256").write_text(f"{manifest['files'][0]['sha256']}  database.dump.gz\n", encoding="utf-8")
    ...
    monkeypatch.setattr("nxbak.restore.show_binary_file", lambda _root, _commit, path, target: target.write_bytes((tmp_path / path).read_bytes()))
    monkeypatch.setattr("nxbak.restore.database.check_connection", lambda _url: None)
    monkeypatch.setattr("nxbak.restore.database.restore_dump", lambda *_: None)
    
    seen = {}
    def fake_execute_sql(url, sql):
        seen["url"] = url
        seen["sql"] = sql

    monkeypatch.setattr("nxbak.restore.database.execute_sql", fake_execute_sql)

    config = Config(1, "origin", Branches(), SourceConfig(database_url_env="DB_URL"), BackupConfig(encryption=False), RestoreConfig())
    result = restore_snapshot(tmp_path, config, snapshot_type="manual", commit=None, dry_run=False)
    
    assert result["mode"] == "restored"
    assert "sql" in seen
    assert 'CREATE EXTENSION IF NOT EXISTS "uuid-ossp" SCHEMA "extensions";' in seen["sql"]
    assert 'ALTER PUBLICATION supabase_realtime ADD TABLE "public"."solves"' in seen["sql"]
