...
def test_restore_dry_run(monkeypatch, tmp_path: Path):
    ...
    manifest = create_manifest(backup_type="manual", encrypted=False, backup_file=backup, created_at="2026-06-07T02:00:00Z")
    ...
    monkeypatch.setattr("nxbak.restore.database.restore_database", lambda *_: called.update(restore=True))

    config = Config(1, "origin", Branches(), SourceConfig(database_url_env="DB_URL"), BackupConfig(encryption=False), RestoreConfig())
    result = restore_snapshot(tmp_path, config, snapshot_type="manual", commit=None, dry_run=True)
